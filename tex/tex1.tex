...
\href{MySQL Community Downloads}{https://dev.mysql.com/downloads/}

``\textsf{Connector/j}'' をクリックする。

\href{MySQL Community Downloads}{https://dev.mysql.com/downloads/connector/j/} のページに遷移するので、ここのところからインストールするのだが、
``\textsf{Connector/J 8.0.21}''と書かれているところの
``\textsf{Select Operating System...}'' には、\textsf{Windows} はない。
...
``\textsf{Windows (x86, 32 \& 64-bit), MySQL Installer MSI}'' の横の
 ...
 \href{MySQL Community Downloas/MySQL Installer 8.0.21}{https://dev.mysql.com/downloads/connector/j/}
 というページが開き、少し下に ``\textsf{MySQL Installer 8.0.21}'' というコラムがある。
 ``\textsf{Generally Available (GA) Releases}'' とある。

その中に二つの \textsf{Download} ボタンがある。

上は、``\textsf{Windows(x86, 32-bit), MSI Installer 8.0.21 24.5M}'' とある。
ダウンロードファイルは、``\textsf{mysql-installer-web-community-8.0.21.0.msi}''

下は、``\textsf{Windows(x86, 32-bit), MSI Installer 8.0.21 427.6M}'' とある。
ダウンロードファイルは、``\textsf{mysql-installer-community-8.0.21.0.msi}''
...
ダウンロードフォルダに ``\textsf{mysql-installer-community-8.0.21.0.msi}'' がダウンロードされる。
...
``\textsf{Connector/J}'' － ``\textsf{Connector/J 8.0}'' － ``\textsf{Connector/J 8.0.21-X86}''
とクリックすると、右側の矢印が緑色に変わるので、その矢印をクリックする。

すると、右側に ``\textsf{Connector/J 8.0.16-X86}'' が表示される。
``\textsf{Next}'' をクリックする。

``\textsf{Installation}'' のダイアログが開く。
``\textsf{Connector/J 8.0.21}'' が表示されており、``\textsf{Ready to Install}'' となっている。
...
コピーしておいたログをメモ帳などで見てみると、どこにインストールされたかわかるので、のちにクラスパスを指定するときに役立つ。このログは ``\textsf{connector-j.log}'' とでも名前をつけて保存しておく。

\subsection{クラスパスの指定}

たとえば、以下のようにする。

\verb!> set CLASSPATH=.;C:\Program Files (x86)\MySQL\Connector J 8.0\mysql-connector-java-8.0.16.jar!
